...
(radius = document.querySelector("#radius")), (errCount = 0);
...
let page = 0,
  ...
  fuel = "",
  cylinders = "";
(engineFacets = cylinders + engines + fuel + fuelTypes),
  (transmissions = []),
  (transmissionFacets = "" + transmissions),
  (apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`);
...
const sort = document.querySelector("#sort"),
  ...
  automatic = document.querySelector("#automatic"),
  ...
  engine.addEventListener("change", () => {
    if (engine.checked) {
      if (
        ...
        cylinders = "&cylinders=";
        if (fuelTypes.includes("unleaded")) {
          true;
        } else {
          fuelTypes.push("unleaded");
        }
        fuel = "&fuel_type=";
        engineFacets = cylinders + engines + fuel + fuelTypes;
        apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
        ...
        engine.value === "electric" ||
        engine.value === "electric / unleaded"
      ) {
        fuelTypes.push(engine.value);
        fuel = "&fuel_type=";
        engineFacets = cylinders + engines + fuel + fuelTypes;
        apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
        fetchURLs();
      ...
      if (
        ...
        engines = engines.filter(e => e !== engine.value);
        engines.length === 0 ? (cylinders = "") : (cylinders = "&cylinders=");
        engines.length === 0
          ? (fuelTypes = fuelTypes.filter(e => e !== "unleaded"))
          : false;
        engineFacets = cylinders + engines + fuel + fuelTypes;
        apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
        fetchURLs();
        ...
        fuelTypes = fuelTypes.filter(e => e !== engine.value);
        fuelTypes.length === 0 ? (fuel = "") : (fuel = "&fuel_type=");
        engineFacets = cylinders + engines + fuel + fuelTypes;
        apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
        fetchURLs();
      }
...
engineSorter(fourCyl);
engineSorter(sixCyl);
engineSorter(eightCyl);
engineSorter(electric);
engineSorter(hybrid);

// ! Sorting transmissions
const transmissionSorter = transmission => {
  transmission.addEventListener("change", () => {
    if (transmission.checked) {
      transmissions.push(transmission.id);
      transmissionFacets = "&transmission=" + transmissions;
      apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
      fetchURLs();
    } else if (!transmission.checked) {
      transmissions = transmissions.filter(e => e !== transmission.id);
...
transmissionSorter(automatic);
transmissionSorter(manual);
...
async function fetchURLs() {
  ...
  if (storedMake === null || storedModel === null) {
    return (output.innerHTML = `<div class="results-not-found"><h1>Please select a make and model.</h1></div>`);
  }
  ...
  try {
    ...
    if (data[0].listings.length < 1) {
      return (output.innerHTML = `<div class="results-not-found"><h1>No listings found. Please change your filters or try again.</h1></div>`);
    }
    ...
    if (storedMake === "") {
      heading.innerHTML = `Displaying ${data[0].num_found
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",")} cars within ${
        radius.value
      } miles of ${data[1].results[0].locations[0].adminArea5}, ${
        data[1].results[0].locations[0].adminArea3
      }`;
    } else if (storedModel === "") {
      heading.innerHTML = `Displaying ${data[0].num_found
        .toString()
      ...
      } within ${radius.value} miles of ${
        data[1].results[0].locations[0].adminArea5
      }, ${data[1].results[0].locations[0].adminArea3}`;
    } else {
      heading.innerHTML = `Displaying ${data[0].num_found
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",")} results for ${
        data[0].listings[0].build.make
      } ${data[0].listings[0].build.model} within ${radius.value} miles of ${
        data[1].results[0].locations[0].adminArea5
      }, ${data[1].results[0].locations[0].adminArea3}`;
    }
    ...
    await data[0].listings.forEach(element => {
      const stockPhotoLink =
        element.media &&
        element.media.photo_links &&
        element.media.photo_links.length > 0
          ? element.media.photo_links[0]
          : "./img/placeholder.png";
      const price = element.price
        ? element.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        : "Not Available";
      const miles = element.miles
        ? element.miles.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        : "Not Available";
      const owners = element.carfax_1_owner
        ? `<div class="center"> <span class="iconify one-owner-icon" data-icon="mdi-account" data-inline="false"></span> </div>
  <div class="center"> <span class="history-text">One previous owner</span> </div>`
        : `<div class="center"> <span class="iconify multi-owner-icon" data-icon="mdi-account-multiple" data-inline="false"></span> </div>
<div class="center"> <span class="history-text">Multiple owners</span> </div>`;
      const city =
        element.dealer.city && element.dealer.city.length > 0
          ? element.dealer.city
          : "NA";
      const state =
        element.dealer.state && element.dealer.state.length > 0
          ? element.dealer.state
          : "NA";
      const intColor =
        element.interior_color && element.interior_color.length > 0
          ? element.interior_color
          : "NA";
      const extColor =
        element.exterior_color && element.exterior_color.length > 0
          ? element.exterior_color
          : "NA";
      const transmission =
        element.build.transmission && element.build.transmission.length > 0
    ...
    errCount += 1;
    if (errCount >= 3) {
      output.innerHTML = `<div class="results-not-found">It looks like we're having some trouble, sorry about that! Please reload the page and try again.</div>`;
    } else {
      output.innerHTML = `<div class="results-not-found">We're sorry, your request could not be completed. Please try again.</div>`;
    }
  }
}
...
const priceFilter = document.querySelector("#price-filter"),
  ...
  engineDropdown = document.querySelector("#engine-dropdown"),
  transmissionFilter = document.querySelector("#transmission-filter"),
  transmissionDropdown = document.querySelector("#transmission-dropdown");
...
toggleFilter(colorDropdown, colorFilter);
toggleFilter(engineDropdown, engineFilter);
toggleFilter(transmissionDropdown, transmissionFilter);
...
lowerSliderPrice.addEventListener("change", () => {
  lowerPriceRange = parseInt(lowerPrice.innerHTML.replace(/[$,+]/g, ""));
  apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
  if (lowerPriceRange === 0) {
    lowerPriceRange = 1;
    apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
  }
  fetchURLs();
...
upperSliderMiles.addEventListener("change", () => {
  upperMilesRange = parseInt(upperMiles.innerHTML.replace(/[$,+]/g, ""));
  apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
  if (upperMilesRange === 150000) {
    upperMilesRange = 999999;
    apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
  }
  fetchURLs();
...
lowerSliderMiles.addEventListener("change", () => {
  lowerMilesRange = parseInt(lowerMiles.innerHTML.replace(/[$,+]/g, ""));
  apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
  if (lowerMilesRange === 150000) {
    lowerMilesRange = 999999;
    apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`;
  }
  fetchURLs();
});
...
upperSliderYear.addEventListener("change", () => {
  upperYearRange = parseInt(upperYear.innerHTML.replace(/[$,+]/g, ""));
  (years = range(lowerYearRange, upperYearRange)),
    (apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`);
  fetchURLs();
...
lowerSliderYear.addEventListener("change", () => {
  lowerYearRange = parseInt(lowerYear.innerHTML.replace(/[$,+]/g, ""));
  (years = range(lowerYearRange, upperYearRange)),
    (apiUrl = `https://marketcheck-prod.apigee.net/v1/search?api_key=${apiKey}&car_type=used&make=${storedMake}&model=${storedModel}&zip=${storedZip}&price_range=${lowerPriceRange}-${upperPriceRange}&miles_range=${lowerMilesRange}-${upperMilesRange}&year=${years}&carfax_clean_title=true&radius=${radius.value}&start=${page}&rows=10${sortFacets}${colorFacets}${engineFacets}${transmissionFacets}`);
  fetchURLs();
});
